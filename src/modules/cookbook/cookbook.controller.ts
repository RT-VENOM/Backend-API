...
import { Recipe } from "./cookbook.model";
...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const user = await CookbookUser.findOne({
...
export const getMe = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const user = await CookbookUser.findById(decoded.userId).select(
      "-passwordHash",
    );
    if (!user) {
...
export const createRecipe = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    // 1. Ensure Multer actually caught an image
    if (!req.file) {
      res.status(400).json({ error: "Image is required" });
      return;
    }

    // 2. Grab the Cloudinary URL generated by Multer
    const imageUrl = req.file.path; 
    
    // 3. Grab the text data from the frontend
    ...
    const newRecipe = await Recipe.create({
      ...
      category,
      imageUrl,
      authorId: req.user?.userId, // We got this from the requireAuth middleware!
    });

    res.status(201).json({ message: "Recipe published!", recipe: newRecipe });
  } catch (error: any) {
...
export const updateAvatar = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const updatedUser = await CookbookUser.findByIdAndUpdate(
      req.user?.userId,
      { avatar: avatarUrl },
      { new: true } // Returns the updated document
    ).select("-passwordHash"); // Exclude password for security
